...
static struct io_plan *peer_connected_out(struct io_conn *conn,
					  ...
					  struct json_defer *defer)
{
	struct json_result *response;
	struct peer *peer = new_peer(state, conn, SOCK_STREAM, IPPROTO_TCP,
				     "out");
	if (!peer) {
		command_fail(defer->cmd, "Failed to make peer");
		return io_close(conn);
	}
	log_info(peer->log, "Connected out to %s:%s", defer->name, defer->port);

	response = new_json_result(defer);	
	json_object_start(response, NULL);
	json_object_end(response);
	command_success(defer->cmd, response);
...
static void peer_failed(struct lightningd_state *state, struct json_defer *defer)
{
	/* FIXME: Better diagnostics! */
	command_fail(defer->cmd, "Failed to connect to peer");
}
			...
			const char *buffer, const jsmntok_t *params)
{
	...
	defer = tal(cmd, struct json_defer);
	defer->cmd = cmd;
	defer->name = tal_strndup(defer, buffer + host->start,
				  host->end - host->start);
	defer->port = tal_strndup(defer, buffer + port->start,
				  port->end - port->start);
	if (!dns_resolve_and_connect(cmd->state, defer->name, defer->port,
				     peer_connected_out, peer_failed, defer)) {
